...
# New logic added - A 2nd cutoff value is added and the signal is compared to square or triangle signal.
# Main cutoff still 8 hz
# 2nd cutoff at 2 hz
...
output_csv_path = 'Data_online_18_FEB_Test.csv'
...
# Correlation
# ref_flat = np.ones(40)
ref_flat = np.full(40, 1) + np.random.normal(0, 0.0005, 40)  # Almost flat
...
def safe_pearsonr(x, y):
    if np.std(x) == 0 or np.std(y) == 0:
        return 0.0  # or np.nan or some fallback
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", category=RuntimeWarning)
        return pearsonr(x, y)[0]

def compare_signals(signal, ref_flat, ref_triangle):
    corr_flat = safe_pearsonr(signal, ref_flat)
    corr_triangle = safe_pearsonr(signal, ref_triangle)
    return corr_flat, corr_triangle
...
try:
    ...
    while acquisition_running:
        ...
        # Delay before starting processing
        ...
        if processing_started and len(live_buffer) >= window_size:
            ...
            if current_time - last_prediction_time >= 0.05:
                ...
                if len(live_buffer) >= 40:
                    ...
                    # To get the 2nd signal with different cutoff value
                    # Apply Butterworth filter to the triboelectric signal LA
                    filtered_LA_2 = butter_lowpass_filter(
                        last_samples['Capteur Triboélectrique LA'], 2, sampling_rate
                    )
                    # Apply Butterworth filter to the triboelectric signal RA
                    filtered_RA_2 = butter_lowpass_filter(
                        last_samples['Capteur Triboélectrique RA'], 2, sampling_rate
                    )
                    # Apply Butterworth filter to the triboelectric signal BA
                    filtered_BA_2 = butter_lowpass_filter(
                        last_samples['Capteur Triboélectrique BA'], 2, sampling_rate
                    )

                    # Normalize the signal

                    
                    for name, sig in zip(['LA', 'RA', 'BA'], [norm_LA, norm_RA, norm_BA]):
                            flat_corr, triangle_corr = compare_signals(sig, ref_flat, ref_triangle)
                            # signal_type = 'flat' if flat_corr > triangle_corr else 'triangle'
                            signal_type = 'flat' if abs(flat_corr) > abs(triangle_corr) else 'triangle'

                            correlation_history[name].append(signal_type)
            ...
            # Check if any recent value in filtered signals is above 500
            high_value_detected = (
                any(val > 400 for val in filtered_LA) or
                any(val > 400 for val in filtered_RA) or
                any(val > 400 for val in filtered_BA)
            )

            # Start the 2-second timer if a high value is detected and we haven't already started
            if high_value_detected and trigger_time is None:
                trigger_time = time.time()

            # If 2 seconds have passed since the trigger, evaluate the situation
            if trigger_time is not None and current_time - trigger_time >= 2.0:
                trigger_time = None  # Reset the trigger

                # Calculate the mean to identify the most affected sensor
                mean_RA = np.mean(filtered_RA) if filtered_RA.size > 0 else 0
                mean_LA = np.mean(filtered_LA) if filtered_LA.size > 0 else 0
                mean_BA = np.mean(filtered_BA) if filtered_BA.size > 0 else 0
                ...
                if sensor_means[max_sensor] > touch_threshold:
                    # Count only the last 36 correlation results
                    recent_corrs = correlation_history[max_sensor][-36:]
                    count_flat = recent_corrs.count('flat')
                    count_triangle = recent_corrs.count('triangle')
                    overall_corr = 'flat' if count_flat > count_triangle else 'triangle'
                    ...
                    if most_common:
                        print(f"{max_sensor}: {most_common[0][0]}")
                    print(f"   ↳  {overall_corr} ({count_flat} flat vs {count_triangle} triangle)")

                    # Reset history for that sensor
                    correlation_history[max_sensor].clear()
