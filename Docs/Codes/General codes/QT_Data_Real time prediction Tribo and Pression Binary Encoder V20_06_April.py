        ...
        if processing_started and len(live_buffer) >= window_size:
            ...
            if current_time - last_prediction_time >= 0.05:
                ...
                if len(live_buffer) >= 40:
                    ...
                    norm_LA = zscore(filtered_LA_2)
                    norm_RA = zscore(filtered_RA_2)
                    norm_BA = zscore(filtered_BA_2)
            ...
            if current_time - last_summary_time >= 2.0:
               ...
               # Compute mean values for filtered signals
                ...
                count_flat = correlation_history[max_sensor].count('flat')
                count_triangle = correlation_history[max_sensor].count('triangle')

                # Count only the last 19 correlation results
